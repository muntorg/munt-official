...
#define T(expected, seed, data) BOOST_CHECK_EQUAL(MurmurHash3(seed, ParseHex(data)), expected)
    ...
    // but is useful for those trying to implement Gulden libraries as a
...
BOOST_AUTO_TEST_CASE(siphash)
{
    ...
    for (uint8_t x = 0; x < ARRAYLEN(siphash_4_2_testvec); x += 8) {
        ...
        hasher3.Write(uint64_t(x) | (uint64_t(x + 1) << 8) | (uint64_t(x + 2) << 16) | (uint64_t(x + 3) << 24) | (uint64_t(x + 4) << 32) | (uint64_t(x + 5) << 40) | (uint64_t(x + 6) << 48) | (uint64_t(x + 7) << 56));
    }
}
