...
bool FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight, bool fFlushPartialSync) {
    ...
    if (fDoFullFlush) {
        ...
        if (!pcoinsTip->Flush(chainActive.Tip() ? (GetAdjustedTime() - chainActive.Tip()->GetBlockTime() > (4 * 60 * 60) ): false))
...
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    int64_t nStart = GetTimeMicros();
    {
        ...
        bool flushed = view.Flush(chainActive.Tip() ? (GetAdjustedTime() - chainActive.Tip()->GetBlockTime() > (4 * 60 * 60)) : false);
        assert(flushed);
    }
