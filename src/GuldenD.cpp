...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  Gulden-Daemon [options]                     " + _("Start Bitcoin Core Daemon") + "\n";
    ...
    try
    {
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in Gulden-Daemon anymore. Use the Gulden-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
