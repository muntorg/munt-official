...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 9232, 9924));
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        ...
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  Gulden-cli [options] <command> [params]  " + _("Send command to Bitcoin Core") + "\n" +
                  "  Gulden-cli [options] help                " + _("List commands") + "\n" +
                  "  Gulden-cli [options] help <command>      " + _("Get help for a command") + "\n";
