...
import com.novo.unity_wallet.*
import com.novo.unity_wallet.R
import com.novo.unity_wallet.ui.AddressBookAdapter
import com.novo.unity_wallet.util.AppBaseFragment
import com.novo.unity_wallet.util.SwipeToDeleteCallback
...
class SendFragment : AppBaseFragment(), UnityCore.Observer {
                    ...
                    Response.Listener { response ->
                        try
                        {
                            ...
                            if (jsonResponse.getInt("status_code") == 200)
                            {
                                ...
                                val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://blockhut.com/sellnovo.php?sessionid=%s".format(sessionID)))
    ...
    {
        // Enable clipboard button if it contains a valid IBAN, Novo address or Uri
        val text = clipboardText()
        try {
            ...
            if (resultCode == CommonStatusCodes.SUCCESS)
            {
                if (data != null) {
                    ...
                    val recipient = try {
                        ...
                        errorMessage(getString(R.string.not_novo_qr, qrContent))
                        return
                    }
                    SendCoinsFragment.newInstance(recipient, false).show(requireActivity().supportFragmentManager, SendCoinsFragment::class.java.simpleName)
                }
