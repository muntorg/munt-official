...
import com.novo.jniunifiedbackend.AddressRecord
import com.novo.jniunifiedbackend.UnifiedBackend
import com.novo.jniunifiedbackend.UriRecipient
import com.novo.wallet.Config.Companion.PRECISION_SHORT
import com.novo.wallet.R.layout.text_input_address_label
import com.novo.wallet.ui.getDisplayDimensions
...
class SendCoinsFragment : BottomSheetDialogFragment(), CoroutineScope
{
    ...
    private fun performAuthenticatedPayment(d : Dialog, request : UriRecipient, msg: String?, substractFee: Boolean = false)
    {
        ...
        Authentication.instance.authenticate(this@SendCoinsFragment.activity!!,
                null, msg = message) {
            try {
                UnifiedBackend.performPaymentToRecipient(request, substractFee)
                d.dismiss()
            }
    ...
    private fun confirmAndCommitGuldenPayment()
    {
        ...
        try {
            ...
            val fee = UnifiedBackend.feeForRecipient(paymentRequest)
            val balance = UnifiedBackend.GetBalance()
            if (fee > balance) {
    ...
    private fun setAddressLabel(label : String)
    {
        ...
        UnityCore.instance.walletReady.invokeNowOrOnSuccesfullCompletion(this) {
            if (label.isNotEmpty())
            {
                val isInAddressBook = UnifiedBackend.getAddressBookRecords().count { it.name.equals(other = label, ignoreCase = true) } > 0
                if (isInAddressBook) {
