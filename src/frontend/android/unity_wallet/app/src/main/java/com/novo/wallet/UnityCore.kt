...
import com.novo.jniunifiedbackend.*
...
class UnityCore {
    ...
    fun startCore() {
        if (!started) {
            ...
            thread(true)
            {
                ...
                buildInfo = UnifiedBackend.BuildInfo()
                Log.i(TAG, "Unity library loaded: $buildInfo")
                UnifiedBackend.InitUnityLib(cfg.dataDir, cfg.apkPath, cfg.staticFilterOffset, cfg.staticFilterLength, cfg.testnet, false, coreLibrarySignalHandler, "")
            }
    ...
    val progressPercent: Float
        get() {
            return UnifiedBackend.getUnifiedProgress() * 100
        }
    ...
    private val coreLibrarySignalHandler = object : UnifiedFrontend() {
        ...
        override fun notifyCoreReady() {
            ...
            monitorObserversLock.withLock {
                UnifiedBackend.RegisterMonitorListener(monitorHandler)
            }
        }
