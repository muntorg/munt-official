...
import com.novo.wallet.internalErrorAlert
import com.novo.wallet.util.AppBaseActivity
import com.novo.wallet.util.gotoWalletActivity
import com.novo.wallet.util.setFauxButtonEnabledState
...
class EnterRecoveryPhraseActivity : AppBaseActivity(), UnityCore.Observer
{
    ...
    private fun chooseAccessCodeAndProceed(mnemonicPhrase : String)
    {
        ...
        Authentication.instance.chooseAccessCode(this, null) {
            ...
            if (UnityCore.instance.isCoreReady()) {
                if (UnifiedBackend.ContinueWalletFromRecoveryPhrase(mnemonicPhrase, password.joinToString(""))) {
                    gotoWalletActivity(this)
                } else {
                ...
                if (!UnifiedBackend.InitWalletFromRecoveryPhrase(mnemonicPhrase, password.joinToString("")))
    ...
    private fun promptUserForBirthDate()
    {
        ...
        builder.setTitle(getString(R.string.wallet_birth_heading))
                .setItems(R.array.wallet_birth_date) { dialog, selection ->
                    ...
                    when (selection)
                    {
                        0 -> chooseAccessCodeAndProceed(UnifiedBackend.ComposeRecoveryPhrase(recoveryPhrase, unixTime-(86400*31)))
                        1 -> chooseAccessCodeAndProceed(UnifiedBackend.ComposeRecoveryPhrase(recoveryPhrase, unixTime-(86400*365)))
    ...
    {
        if (!UnifiedBackend.IsValidRecoveryPhrase(recoveryPhrase))
        {
    ...
    fun updateView()
    {
        ...
        setFauxButtonEnabledState(recover_from_phrase_proceed_button, UnifiedBackend.IsValidRecoveryPhrase(recoveryPhrase))
    }
