...
import com.novo.wallet.main_activity_fragments.*
import com.novo.wallet.ui.monitor.NetworkMonitorActivity
        ...
        SharedPreferences.OnSharedPreferenceChangeListener
{
    ...
    fun performLink(linkURI: String)
    {
        Authentication.instance.authenticate(this, null, getString(R.string.link_wallet_auth_desc)) { password ->
            ...
            thread(start = true)
            {
                if (!UnifiedBackend.ReplaceWalletLinkedFromURI(linkURI, password.joinToString("")))
                {
                    runOnUiThread {
    ...
    {
        val intent = Intent(Intent.ACTION_VIEW, Uri.parse(Config.PURCHASE_TEMPLATE.format(UnifiedBackend.GetReceiveAddress().toString())))
