...
import com.novo.jniunifiedbackend.LegacyWalletResult
import com.novo.wallet.Constants.OLD_WALLET_PROTOBUF_FILENAME
...
class UpgradeActivity : AppBaseActivity(), UnityCore.Observer
{
    ...
    private fun onUpgradeWithPassword(view : View, oldPassword : String)
    {
        thread(true)
        {
            val result = UnifiedBackend.isValidAndroidLegacyProtoWallet(filesDir.toString() + File.separator + OLD_WALLET_PROTOBUF_FILENAME, oldPassword)
            try
            {
                when (result)
                {
    ...
    fun onUpgrade(view: View)
    {
        ...
        thread(true)
        {
            when (UnifiedBackend.isValidAndroidLegacyProtoWallet(filesDir.toString() + File.separator + OLD_WALLET_PROTOBUF_FILENAME, ""))
            {
        ...
        this.runOnUiThread {
            Authentication.instance.chooseAccessCode(this, getString(R.string.access_code_choose_upgrade_title)) { accessCode ->
                thread(true) {
                    ...
                    UnifiedBackend.InitWalletFromAndroidLegacyProtoWallet(filesDir.toString() + File.separator + OLD_WALLET_PROTOBUF_FILENAME, oldPassword, newPassword)
                }
