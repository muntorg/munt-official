...
package com.novo.wallet.main_activity_fragments
...
import com.novo.wallet.*
import com.novo.wallet.util.invokeNowOrOnSuccesfullCompletion
...
class WalletSettingsFragment : androidx.preference.PreferenceFragmentCompat(), CoroutineScope {
    ...
    override fun onCreatePreferences(savedInstance: Bundle?, rootKey: String?) {
        ...
        UnityCore.instance.walletReady.invokeNowOrOnSuccesfullCompletion(this) {
            if (UnifiedBackend.IsMnemonicWallet()) {
        ...
        when (preference?.key) {
            ...
            "preference_change_pass_code" -> {
                Authentication.instance.authenticate(activity!!, getString(R.string.change_passcode_auth_title), getString(R.string.change_passcode_auth_desc)) { oldPassword ->
                    Authentication.instance.chooseAccessCode(activity!!, getString(R.string.change_passcode_auth_title)) { newPassword ->
                        if (!UnifiedBackend.ChangePassword(oldPassword.joinToString(""), newPassword.joinToString(""))) {
            ...
            "preference_remove_wallet", "preference_unlink_wallet" -> {
                ...
                Authentication.instance.authenticate(activity!!, getString(R.string.remove_wallet_auth_title), msg) {
                    UnifiedBackend.EraseWalletSeedsAndAccounts()
        ...
        if (requestCode == WalletSettingsFragment.REQUEST_CODE_SCAN_FOR_LINK) {
            if (resultCode == CommonStatusCodes.SUCCESS) {
                if (data != null) {
                    ...
                    if (!UnifiedBackend.IsValidLinkURI(barcode.displayValue)) {
                        AlertDialog.Builder(context!!)
                                .setTitle(getString(com.novo.wallet.R.string.no_guldensync_warning_title))
                                .setMessage(getString(com.novo.wallet.R.string.no_guldensync_warning))
                                .setPositiveButton(getString(com.novo.wallet.R.string.button_ok)) { dialogInterface, i ->
                    ...
                    fun performDialog(titleResId: Int, msgResId: Int, withCancel: Boolean, action: (dialogInterface: DialogInterface) -> Unit) {
                        val builder = AlertDialog.Builder(context!!)
                                ...
                                .setCancelable(true)
                        if (withCancel)
                            builder.setNegativeButton(getString(com.novo.wallet.R.string.button_cancel)) { dialogInterface, i ->
                                dialogInterface.dismiss()
                            }
                        builder.create().show()
                    }

                    if (UnifiedBackend.HaveUnconfirmedFunds()) {
                    ...
                    performDialog(
                            ...
                            if (UnifiedBackend.GetBalance() > 0) R.string.guldensync_info_message_non_empty_wallet else R.string.guldensync_info_message_empty_wallet, true
                    ) {
                        it.dismiss()
