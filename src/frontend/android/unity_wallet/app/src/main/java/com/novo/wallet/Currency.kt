...
import com.novo.wallet.Config.Companion.RATE_FETCH_INTERVAL
...
private const val MARKET_URL = "https://api.gulden.com/api/v1/ticker"
...
private suspend fun reallyfetchAllCurrencyRates(): Map<String, Double> {
    ...
    try {
        data  = withContext(Dispatchers.IO) {
            URL(MARKET_URL).readText()
        }
    }
    catch (e: Throwable) {
        Log.i(TAG, "Failed to fetch currency rates from $MARKET_URL")
        throw e
    }
