...
import com.novo.unity_wallet.Config.Companion.RATE_FETCH_INTERVAL
...
private const val NOVO_MARKET_URL = "https://api.novocurrency.com/api/v1/ticker"
 ...
 * @return conversion rate to convert from novo to specified currency (ie. value = novo * rate)
 */
...
private suspend fun reallyfetchAllCurrencyRates(): Map<String, Double> {
    ...
    try {
        data  = withContext(Dispatchers.IO) {
            URL(NOVO_MARKET_URL).readText()
        }
    }
    catch (e: Throwable) {
        Log.i(TAG, "Failed to fetch currency rates from $NOVO_MARKET_URL")
        throw e
    }
...
{
    return "%s %s".format(if (useNativePrefix) "N" else "",
