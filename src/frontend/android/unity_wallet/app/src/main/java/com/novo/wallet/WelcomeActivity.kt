...
import com.novo.wallet.ui.EnterRecoveryPhraseActivity
import com.novo.wallet.ui.ShowRecoveryPhraseActivity
...
class WelcomeActivity : AppBaseActivity(), UnityCore.Observer
{
    ...
    fun onCreateNewWallet(view: View)
    {
        thread(true)
        {
            val recoveryPhrase = UnifiedBackend.GenerateRecoveryMnemonic()
            this.runOnUiThread()
            {
        ...
        if (requestCode == REQUEST_CODE_SCAN_FOR_LINK)
        {
            if (resultCode == CommonStatusCodes.SUCCESS && data != null)
            {
                ...
                Authentication.instance.chooseAccessCode(this, null) {
                    ...
                    {
                        if (UnifiedBackend.ContinueWalletLinkedFromURI(barcode.displayValue, password.joinToString("")))
                        {
                    ...
                    else
                    {
                        ...
                        if (UnifiedBackend.InitWalletLinkedFromURI(barcode.displayValue, password.joinToString("")))
                        {
                            return@chooseAccessCode
                        }
                    }
                    ...
                    alert(Appcompat,  getString(com.novo.wallet.R.string.no_guldensync_warning),  getString(com.novo.wallet.R.string.no_guldensync_warning_title))
                    {
                        positiveButton(getString(com.novo.wallet.R.string.button_ok))
                        {
                            it.dismiss()
                        }
                        isCancelable = true
