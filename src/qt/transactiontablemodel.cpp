...
QString TransactionTableModel::formatTxType(const TransactionRecord *wtx) const
{
    ...
    case TransactionRecord::WitnessRearrangeRecv:
        return tr("Rearrange locked funds");
        ...
        if ( wtx->fromAccountUUID != boost::uuids::nil_generator()())
        {
            ...
            if (wallet->mapAccountLabels.count(fromUUID) != 0)
            {
                switch(wtx->type)
                {
                    ...
                    case TransactionRecord::WitnessRearrangeRecv:
                        return tr("Rearrange locked funds: %1").arg(QString::fromStdString(wallet->mapAccountLabels[fromUUID]));
                    case TransactionRecord::WitnessChangeKeyRecv:
...
static void NotifyTransactionChanged(TransactionTableModel *ttm, CWallet *wallet, const uint256 &hash, ChangeType status, bool)
{
    // Find transaction in wallet
    std::map<uint256, CWalletTx>::iterator mi = wallet->mapWallet.find(hash);
