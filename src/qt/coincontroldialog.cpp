...
CoinControlDialog::CoinControlDialog(const QStyle *_platformStyle, QWidget *parent) :
    QDialog(parent),
...
void CoinControlDialog::setModel(WalletModel *_model)
{
    ...
    {
        updateView(model->getActiveAccount());
        updateLabelLocked();
...
{
    GUIUtil::setClipboard(GuldenUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    for(const COutput& out : vOutputs) {
        ...
        else */if(ExtractDestination(out.tx->tx->vout[out.i], address))
        {
            CPubKey pubkey;
    ...
    int nDisplayUnit = GuldenUnits::NLG;
    ...
    l2->setText(GuldenUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(GuldenUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(GuldenUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(GuldenUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView(CAccount* forAccount)
{
    ...
    model->listCoins(forAccount, mapCoins);
    ...
    for(const PAIRTYPE(QString, std::vector<COutput>)& coins : mapCoins) {
        ...
        for(const COutput& out : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, GuldenUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, GuldenUnits::format(nDisplayUnit, nSum));
