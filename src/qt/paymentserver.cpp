...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the Gulden: URI contains a payment request, we are not able to detect the
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click Gulden: links
...
//
// OSX-specific way of handling Gulden: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    QList<std::pair<CTxOut, CAmount> > sendingTos = request.getPayTo();
    QStringList addresses;

    for(const PAIRTYPE(CTxOut, CAmount)& sendingTo : sendingTos) {
        ...
        // Gulden amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(sendingTo.first, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(GuldenUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    //fixme: (PHASE5) re-write to use segsig transactions.
