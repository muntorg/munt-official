...
static void TxInErrorToJSON(const uint64_t nTransactionVersion, const CTxIn& txin, UniValue& vErrorsRet, const std::string& strMessage)
{
    ...
    for (unsigned int i = 0; i < txin.segregatedSignatureData.stack.size(); i++) {
        witness.push_back(HexStr(txin.segregatedSignatureData.stack[i].begin(), txin.segregatedSignatureData.stack[i].end()));
    }
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.segregatedSignatureData, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(signingKeyID, CKeyID(), &txConst, i, amount), &serror)) {
