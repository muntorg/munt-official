...
static void next_addresses_sse3(argon2_echo_block *address_block, argon2_echo_block *input_block)
{
    ...
    fill_block_sse3(zero_block, input_block, address_block, 0);

    /*Second iteration of G*/
    fill_block_sse3(zero2_block, address_block, address_block, 0);
}
...
void fill_segment_sse3(const argon2_echo_instance_t *instance, argon2_echo_position_t position)
{
    ...
    if ((0 == position.pass) && (0 == position.slice))
    {
        ...
        if (data_independent_addressing)
        {
            next_addresses_sse3(&address_block, &input_block);
        }
    }
    ...
    for (i = starting_index; i < instance->segment_length; ++i, ++curr_offset, ++prev_offset)
    {
        ...
        if(0 == position.pass)
        {
            fill_block_sse3(state, ref_block, curr_block, 0);
        }
        else
        {
            fill_block_sse3(state, ref_block, curr_block, 1);
        }
    }
}
