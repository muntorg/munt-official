...
static void next_addresses_avx512f(argon2_echo_block *address_block, argon2_echo_block *input_block)
{
    ...
    fill_block_avx512f(zero_block, input_block, address_block, 0);

    /*Second iteration of G*/
    fill_block_avx512f(zero2_block, address_block, address_block, 0);
}
...
void fill_segment_avx512f(const argon2_echo_instance_t *instance, argon2_echo_position_t position)
{
    ...
    if ((0 == position.pass) && (0 == position.slice))
    {
        ...
        if (data_independent_addressing)
        {
            next_addresses_avx512f(&address_block, &input_block);
        }
    }
    ...
    for (i = starting_index; i < instance->segment_length; ++i, ++curr_offset, ++prev_offset)
    {
        ...
        if(0 == position.pass)
        {
            fill_block_avx512f(state, ref_block, curr_block, 0);
        }
        else
        {
            fill_block_avx512f(state, ref_block, curr_block, 1);
        }
    }
}
