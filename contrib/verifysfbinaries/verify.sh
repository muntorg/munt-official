...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'guldencoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[guldencoin-]<version>-[rc[0-9]] (example: guldencoin-0.9.2-rc1)"
   echo "wget output:"
