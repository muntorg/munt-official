...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/debian/manpages/GuldenD.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/debian/manpages/Gulden-cli.1
...
# Source31 - what about Gulden-tx and bench_bitcoin ???
...
The Gulden-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the Gulden-tx utility allows you to create a custom transaction, and
...
mv %{buildroot}%{_bindir}/GuldenD %{buildroot}%{_sbindir}/GuldenD

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/GuldenD 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/GuldenD/GuldenD.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/GuldenD -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/GuldenD.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/Gulden-cli.1
%if %{_buildqt}
...
%attr(0755,root,root) %{_sbindir}/GuldenD
...
%attr(0644,root,root) %{_mandir}/man1/GuldenD.1*
...
%attr(0755,root,root) %{_bindir}/Gulden-cli
%attr(0755,root,root) %{_bindir}/Gulden-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/Gulden-cli.1*
%attr(0644,root,root) %{_mandir}/man5/bitcoin.conf.5*
