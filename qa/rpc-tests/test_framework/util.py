...
def check_json_precision():
    """Make sure json library being used does not lose precision converting NLG values"""
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "gulden.conf"), 'w') as f:
        f.write("regtest=1\n")
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: GuldenD started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    GuldenD_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: GuldenD started, waiting for RPC to come up")
...
def stop_node(node, i):
    ...
    GuldenD_processes[i].wait(timeout=GULDEND_PROC_WAIT_TIMEOUT)
...
def wait_GuldenDs():
    # Wait for all GuldenDs to cleanly exit
    for GuldenD in GuldenD_processes.values():
        GuldenD.wait(timeout=GULDEND_PROC_WAIT_TIMEOUT)
    GuldenD_processes.clear()
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s NLG too low! (Should be %s NLG)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s NLG too high! (Should be %s NLG)"%(str(fee), str(target_fee)))
